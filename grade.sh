...
STUDENTFILE=`find student-submission -name "*ListExamples.java"`
echo $STUDENTFILE
...
# echo $TESTCOUNT
TESTFUNC=`grep "static List<String> filter(List<String> list, StringChecker sc)" $STUDENTFILE`
# echo $TESTFUNC
if [[ -z $TESTFUNC ]]
	then
		echo "The student did not implement the filter function"
		echo "Your grade is 0%, please fix method signature and resubmit."
		exit 1
fi
